...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Nạp Action và Observation vào ngữ cảnh để LLM suy luận ở lượt kế tiếp.
            agent_prompt = (
                f"{agent_prompt}\n\n"
                f"Thought: {thought}\n"
                f"Action: {tool_name}({json.dumps(arguments, ensure_ascii=False)})\n"
                f"Observation từ MCP Server: {obs_str}\n"
                "Hãy tiếp tục suy luận từ Observation trên. Nếu đã đủ dữ liệu, "
                "hãy trả lời người dùng bằng văn bản thay vì gọi lại cùng công cụ."
            )
